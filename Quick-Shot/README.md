...
> ⚠️ **IMPORTANT**: You must use `bindr` (bind on release) for region captures. Using standard `bind` triggers `slurp` while you are still holding down the keys, which causes `slurp` to immediately cancel the selection!

```ini
# Takes a screenshot of the entire output/screen (instant, no click)
bindn = Super+Shift+Ctrl, S, exec, takeshot output

# Takes a screenshot of a specific window (select with mouse)
bindr = Super+Shift, S, exec, takeshot window

# Takes a screenshot of a selected region (select with mouse)
...
```lua
-- Fullscreen screenshot (instant, no click)
hl.bind("SUPER + SHIFT + CTRL + S", hl.dsp.exec_cmd("takeshot output"))
