...
# Use native Hyprland/Wayland tool first (no xrandr warnings)
if command -v hyprctl >/dev/null 2>&1; then
    mapfile -t DETECTED_RES < <(hyprctl monitors -j 2>/dev/null | python3 -c "
import json,sys
...
for m in mons:
    ...
    if m.get('transform',0) in (1,3,5,7):
        w,h=h,w
    print(f'{w}x{h}')
" 2>/dev/null | sort -u -t'x' -k1,1nr -k2,2nr)
elif command -v xrandr >/dev/null 2>&1; then
    # Fallback: xrandr (may warn on pure Wayland — harmless)
    mapfile -t DETECTED_RES < <(xrandr 2>/dev/null | awk '/\*/ {print $1}' | grep 'x' | sort -u -t'x' -k1,1nr -k2,2nr)
fi

if [ ${#DETECTED_RES[@]} -eq 0 ]; then
    echo "⚠️  Could not detect display resolution dynamically."
    echo "Please choose a fallback resolution:"
    ...
    while true; do
        ...
        case "$choice" in
            1) DETECTED_RES=("1920x1080"); break ;;
            2) DETECTED_RES=("2560x1440"); break ;;
            3) DETECTED_RES=("3840x2160"); break ;;
            *) echo "❌ Invalid choice." ;;
        esac
    done
fi
